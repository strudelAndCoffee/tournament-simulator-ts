...
import { PlayerRollType } from '../Player/Player.js'
...
export default class Match {
  run_round(player_1: Player, player_2: Player) {
    const p1_roll = player_1.roll(player_2)
    const p2_roll = player_2.roll(player_1)
  ...
  duel(p1_roll: PlayerRollType, p2_roll: PlayerRollType) {
    let p1 = false
    let p2 = false
    let tie = false
    let p1_crit = false
  ...
  update_players(player_1: Player, player_2: Player, result: RoundResultType) {
    if (result.p1) {
      player_1.xp += 3
      if (result.p1_crit) {
        player_1.xp++
